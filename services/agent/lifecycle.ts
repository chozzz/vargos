...
export class AgentLifecycle extends EventEmitter {
  ...
  startRun(runId: string, sessionKey: string): void {
    ...
    this.activeRuns.set(runId, { sessionKey, startedAt: Date.now(), abortController });

    const event: LifecycleEvent = {
      type: 'lifecycle', phase: 'start', runId, sessionKey, timestamp: Date.now(),
    };
    this.emit('stream', event);
    this.emit('start', runId, sessionKey);
  }

  endRun(runId: string, tokens?: { input: number; output: number; total: number }): void {
    ...
    const event: LifecycleEvent = {
      type: 'lifecycle', phase: 'end', runId, sessionKey: run.sessionKey,
      timestamp: Date.now(), tokens, duration,
    };
    this.emit('stream', event);
  ...
  errorRun(runId: string, error: Error | string): void {
    ...
    const event: LifecycleEvent = {
      type: 'lifecycle', phase: 'error', runId, sessionKey: run.sessionKey,
      timestamp: Date.now(), error: error instanceof Error ? error.message : error, duration,
    };
    this.emit('stream', event);
  ...
  abortRun(runId: string, reason?: string): boolean {
    ...
    const event: LifecycleEvent = {
      type: 'lifecycle', phase: 'abort', runId, sessionKey: run.sessionKey,
      timestamp: Date.now(), error: reason || 'Aborted',
    };
    this.emit('stream', event);
  ...
  streamAssistant(runId: string, content: string, isComplete = false): void {
    const run = this.activeRuns.get(runId);
    if (!run) return;
    const event: AssistantStreamEvent = {
      type: 'assistant', runId, sessionKey: run.sessionKey, content, isComplete, timestamp: Date.now(),
    };
    this.emit('stream', event);
  ...
  streamTool(
    runId: string, toolName: string, phase: 'start' | 'end',
    args?: unknown, result?: unknown, error?: string,
  ): void {
    const run = this.activeRuns.get(runId);
    if (!run) return;
    const event: ToolStreamEvent = {
      type: 'tool', runId, sessionKey: run.sessionKey, toolName, phase, args, result, error, timestamp: Date.now(),
    };
    this.emit('stream', event);
    this.emit('tool', event);
  }

  streamCompaction(runId: string, tokensBefore: number, summary: string): void {
    const run = this.activeRuns.get(runId);
    if (!run) return;
    const event: CompactionStreamEvent = {
      type: 'compaction', runId, sessionKey: run.sessionKey, tokensBefore, summary, timestamp: Date.now(),
    };
    this.emit('stream', event);
