...
export function computeAdaptiveChunkRatio(messages: AgentMessage[], contextWindow: number): number {
  ...
  const safeAvg = (totalTokens / messages.length) * SAFETY_MARGIN;
  const ratio = safeAvg / contextWindow;
  if (ratio > 0.1) {
    return Math.max(MIN_CHUNK_RATIO, BASE_CHUNK_RATIO - Math.min(ratio * 2, BASE_CHUNK_RATIO - MIN_CHUNK_RATIO));
  }
...
export async function summarizeInStages(params: {
  ...
  return summarizeWithFallback({
    ...params, messages: partials.map(s => userMessage(s)), customInstructions: mergeInstructions,
  });
}

// ── Metadata collection ────────────────────────────────────────────────────────

interface ToolFailure { toolName: string; summary: string }
            ...
            if (pruned.dropped.length > 0) {
              ...
              try {
                ...
                droppedSummary = await summarizeInStages({
                  ...
                  maxChunkTokens: maxChunk, contextWindow, customInstructions,
