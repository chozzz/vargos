...
/**
 * Check if current UTC hour is within active hours.
 * Returns true if no config (always active).
 * Supports overnight ranges (e.g. 22→6).
 *
 * @param activeHours [startHour, endHour] in UTC (0-23), e.g. [8, 18]
 */
export function isWithinActiveHours(activeHours?: [number, number]): boolean {
