...
export class McpEdge {
    ...
    constructor(bus, config) {
        this.bus = bus;
        this.config = config;
        this.mcpServer = new Server({ name: 'vargos', version: VERSION }, { capabilities: { tools: {} } });
        this.setupHandlers();
    }
    httpHost() {
        ...
        this.mcpServer.setRequestHandler(ListToolsRequestSchema, async () => {
            ...
            return {
                tools: tools.map(m => ({
                    ...
                    inputSchema: (m.schema?.params ?? {}),
                })),
            };
        });
        this.mcpServer.setRequestHandler(CallToolRequestSchema, async (request) => {
            const { name, arguments: args } = request.params;
            const sessionKey = args?.sessionKey || 'mcp:default';
