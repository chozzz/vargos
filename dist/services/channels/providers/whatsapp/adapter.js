...
export class WhatsAppAdapter extends BaseChannelAdapter {
    ...
    sock = null;
    botJid = '';
    botLid = null; // learned from proper mentions
    reconnector = new Reconnector();
    reconnectTimer = null;
    authDir = '';
    constructor(instanceId, deps, allowFrom) {
    ...
    async start() {
        ...
        if (!existsSync(creds)) {
            this.status = 'error';
            throw new Error(`No auth state found at ${this.authDir} — run "vargos channels register whatsapp ${this.instanceId}" to pair`);
        }
        try {
    ...
    async sendMedia(sessionKey, filePath, mimeType, caption) {
        if (!this.sock)
            throw new Error('WhatsApp not connected');
    ...
    handleInbound(msg) {
        if (msg.fromMe)
            return;
        if (!this.dedupe.add(msg.messageId))
            return;
    ...
    async handleMedia(msg, normalizedMsg) {
        ...
        if (!msg.mediaBuffer) {
            ...
            const messageWithText = { ...normalizedMsg, text: content };
            await this.onInboundMessage(sessionKey, messageWithText);
            return;
        }
        const { caption, savedPath, mimeType } = await this.processInboundMedia(msg, (text) => this.onInboundMessage(sessionKey, { ...normalizedMsg, text }), sessionKey, normalizedMsg.chatType !== 'group');
