...
| Transport abstraction | Connection handling behind a `ConnectionTransport` interface — WebRTC DataChannel swap is a transport-layer change only (see `plans/webrtc-gateway.md`) |
...
- [ ] `src/web/service.ts`: ServiceClient, HTTP+SSE on port 9003
- [ ] `ConnectionTransport` interface — SSE today, WebRTC DataChannel when needed
