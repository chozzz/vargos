# Planned: Cron-Triggered Outbound Voice Call
...
- Phase 1 (Voice Foundation) + Phase 3 (Outbound Calls)
- Fast local LLM for real-time voice (Claude too slow at ~400ms budget)
- Max call duration and sync vs. async pattern decision
...
- Cron notify delivery must fire only once (see KNOWN_ISSUES.md: subagent completion storm)
