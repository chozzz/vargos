/**
 * Sessions list tool - Updated to use service abstraction
  ...
  execute: async (args: unknown, context: ToolContext) => {
    ...
    try {
      ...
      const list = await sessions.list({
        limit: params.limit,
        kind: params.kinds?.[0], // Service supports single kind filter
      });
      ...
      const formatted = await Promise.all(list.map(async s => {
        ...
        if (messageLimit > 0) {
          const messages = await sessions.getMessages(s.sessionKey, { limit: messageLimit });
