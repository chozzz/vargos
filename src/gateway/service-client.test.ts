...
describe('ServiceClient', () => {
  ...
  it('rejects pending requests on disconnect', async () => {
    ...
    const promise = caller.call('echo', 'echo.ping', { n: 1 });

    // Disconnect before response can arrive
    await caller.disconnect();

    await expect(promise).rejects.toThrow();
  });

  it('throws not connected when calling after disconnect', async () => {
    const echo = await createEcho();
    await echo.disconnect();
  ...
  it('emit silently ignores when not connected', async () => {
    const echo = new EchoService({
      events: TEST_EVENT,
    });
    // Never connect — emit should not throw
    expect(() => echo.emit('test.event', { data: 1 })).not.toThrow();
  });
});
