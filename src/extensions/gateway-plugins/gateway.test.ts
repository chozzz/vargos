...
import { Gateway, PluginRegistry, TextInputPlugin, ImageInputPlugin, MediaInputPlugin, getGateway, initializeGateway, type GatewayContext, type GatewayResponse, type NormalizedInput } from '../../core/gateway/index.js';
import { processAndDeliver } from '../../core/gateway/core.js';

vi.mock('../../core/lib/media.js', () => ({
  saveMedia: vi.fn().mockResolvedValue('/tmp/mock-saved-path.jpg'),
}));

vi.mock('../../core/services/factory.js', () => {
  const mockSessions = {
...
describe('Gateway execute flow', () => {
  ...
  it('text input → TextPlugin.prepare → runtime.run with text only', async () => {
    const { getPiAgentRuntime } = await import('../../core/runtime/runtime.js');
    const { getSessionService } = await import('../../core/services/factory.js');
