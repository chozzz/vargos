...
export function setupSwagger(app: INestApplication, port: number | string) {
  ...
  setTimeout(() => {
    ...
    Logger.verbose(`Swagger UI is available at: http://localhost:${port}/api/swagger`, 'Swagger');
    Logger.verbose(`Swagger JSON is available at: http://localhost:${port}/api/json`, 'Swagger');
  }, 1e3);
}